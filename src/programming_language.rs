...
impl<'a> SExpr<'a> {
    fn parse_expr(input: &'a str) -> Result<(Vec<Self>, usize), Vec<String>> {
        ...
        while idx < input.len() {
            ...
            let literal_len = consume_chars(&input[idx..], |&c| c.is_alphanumeric());
            if literal_len > 0 {
    ...
    fn parse(input: &'a str) -> Result<Self, Vec<String>> {
        if input.len() < 1 || &input[0..1] != "(" {
            ...
            let token_len = consume_chars(&input[non_space_start..], |&c| c.is_alphanumeric());
            let space_after_tok = consume_chars(&input[non_space_start + token_len..], |&c| c.is_whitespace());
            if non_space_start + token_len + space_after_tok != input.len() {
                return Err(vec!["Invalid token!".into()]);
            }
            return Ok(SExpr::Literal(&input[non_space_start..non_space_start + token_len]));
        }
